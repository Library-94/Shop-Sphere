    ...
    <header className="w-full sticky top-0 z-50">
      ...
      <div className="bg-amazon text-white flex items-center h-[60px] px-4 gap-2 sm:gap-4">
        ...
        <form className="flex-1 flex h-10 rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-amazon-orange" onSubmit={handleSearch}>
          <div className="bg-gray-100 text-gray-600 px-3 flex items-center text-xs border-r border-gray-300 hover:bg-gray-200 cursor-pointer rounded-l-md">
            All <svg className="w-2 h-2 ml-1 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/></svg>
          </div>
          <input 
            type="text" 
      ...
      <div className="bg-amazon-light text-white text-sm flex items-center h-[40px] px-4 gap-4 overflow-x-auto hide-scrollbar whitespace-nowrap">
        <div className="flex items-center cursor-pointer hover:border hover:border-white p-1 font-bold">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
          All
        </div>
        {['Fresh', 'Sell', 'Bestsellers', 'Mobiles', "Today's Deals", 'Customer Service', 'Electronics', 'New Releases', 'Home & Kitchen'].map((item) => (
          <span key={item} className="cursor-pointer hover:border hover:border-white p-1 px-2 rounded-sm">{item}</span>
        ))}
